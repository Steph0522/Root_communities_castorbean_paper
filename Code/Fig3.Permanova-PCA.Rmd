...
meta$Compartment<- factor(meta$Type_of_soil,
                   ...
                   labels = c("Bulk soil", "Rhizosphere",
                              "Roots", "Uncultivated"))
meta$Water_regime<- factor(meta$Treatment,
                        levels = c( "0", "1", "2", "3"),
...
pca<- ggplot() +
  ...
  geom_point(                              
    ...
    aes(x=PC1, y=PC2, color=Compartment, shape=Water_regime), 
  ...
  fontface="italic",  box.padding = 0.6, size=4)+ theme(
...
ad_grouped<-adonis2(d.clr.abund2~Water_regime*Compartment,
...
Permanova_grouped<-data.frame(ad_grouped, check.names = F) %>% rownames_to_column(
...
metadata$Water_regime<- factor(metadata$Treatment,
                       levels = c( "0", "1", "2", "3"),
...
table_all_ro<- table_filter %>% dplyr::select_at(vars(
  ...
  var = "#SampleID") %>% inner_join(metadata)  %>% group_by(
    Water_regime) %>% summarise_at(c(2:152), mean) %>% column_to_rownames(
      var = "Water_regime") %>% t()
...
all_ro<- table_filter %>% dplyr::select_at(vars(
  ...
  var = "#SampleID") %>% inner_join(metadata) %>%arrange(
    Water_regime) %>% filter(
    !Water_regime=="T0") %>%  dplyr::select(
    -BarcodeSequence:-Water_regime) %>% column_to_rownames(
      var = "#SampleID") %>% t()
...
pca_tables2<- function(tab){ggplot() +
    ...
    geom_point(data=data.frame(tab$x) %>%
               ...
               aes(x=PC1, y=PC2, fill=Water_regime),shape=21, size=4) +
...
a5<-adonis2(clr_perma[[1]]~Water_regime, data = meta_just_list2[[1]], method = 
              "euclidian", permutations =perm) %>% round(., digits = 2) %>%replace(
                is.na(.), "-")


setBlocks(perm)<- with(meta_just_list2[[2]], Plot)
a6<-adonis2(clr_perma[[2]]~Water_regime, data = meta_just_list2[[2]], method = 
              "euclidian", permutations =perm) %>% round(., digits = 2) %>%replace(
                is.na(.), "-")
setBlocks(perm)<- with(meta_just_list2[[3]], Plot)
a7<-adonis2(clr_perma[[3]]~Water_regime, data = meta_just_list2[[3]], method =
              "euclidian", permutations =perm) %>% round(., digits = 2) %>%replace(
                is.na(.), "-")
...
pca_allthree<- plot_grid(NULL,legends, NULL, 
                 pca_ro_all+theme(legend.position = "none"),
                 pca_ri_all+theme(legend.position = "none"), 
                 pca_nr_all+theme(legend.position = "none"), 
                 nrow = 2, ncol = 3, rel_heights = c(0.5,2),
                 labels = c("","","","C                    Roots",
                            "E            Rhizosphere", 
                            "G               Bulk soil"), 
                 rel_widths = c(1,1,1), label_x = -0.16, 
                 label_y = 1, hjust = -0.5, vjust = -0.5 )
